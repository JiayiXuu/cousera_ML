...
cost = sum((-Y .* log(a3)) - ((1 - Y) .* log(1 - a3)), 2); % cost(5000, 1);
J = (1 / m) * sum(cost);
...
J = J + (lambda / (2 * m )) * (sum(sum(Theta1_2 .^ 2)) + sum(sum(Theta2_2 .^ 2)));
...
for t = 1:m
	...
	d2 = (Theta2_2' * d3) .* sigmoidGradient(z2);
...
Theta1_grad(:,2:end) += (lambda / m) * Theta1_2;
Theta2_grad(:,2:end) += (lambda / m) * Theta2_2;
%
